...
func (c *eventLogServiceClient) EventFetched(ctx context.Context, opts ...grpc.CallOption) (EventLogService_EventFetchedClient, error) {
	stream, err := c.cc.NewStream(ctx, &EventLogService_ServiceDesc.Streams[1], "/proto.EventLogService/EventFetched", opts...)
	if err != nil {
		return nil, err
	}
	x := &eventLogServiceEventFetchedClient{stream}
	return x, nil
}
...
type eventLogServiceEventFetchedClient struct {
	grpc.ClientStream
}

func (x *eventLogServiceEventFetchedClient) Send(m *EventFetchedRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *eventLogServiceEventFetchedClient) CloseAndRecv() (*EventFetchedResponse, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(EventFetchedResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
func (UnimplementedEventLogServiceServer) EventFetched(EventLogService_EventFetchedServer) error {
	return status.Errorf(codes.Unimplemented, "method EventFetched not implemented")
}
...
func _EventLogService_EventFetched_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EventLogServiceServer).EventFetched(&eventLogServiceEventFetchedServer{stream})
}
...
type eventLogServiceEventFetchedServer struct {
	grpc.ServerStream
}

func (x *eventLogServiceEventFetchedServer) SendAndClose(m *EventFetchedResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *eventLogServiceEventFetchedServer) Recv() (*EventFetchedRequest, error) {
	m := new(EventFetchedRequest)
