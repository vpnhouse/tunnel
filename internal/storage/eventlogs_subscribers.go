...
func (storage *Storage) PutEventlogsSubscriber(eventlogSubscriber *types.EventlogSubscriber) error {
	now := xtime.Now()
	eventlogSubscriber.Updated = &now
	query := `
...
`
	_, err := storage.db.NamedExec(query, eventlogSubscriber)
	if err != nil {
		return xerror.EStorageError("can't put eventlog subscriber data", err, zap.Any("eventlog_subscriber", eventlogSubscriber))
	}
	return nil
}
